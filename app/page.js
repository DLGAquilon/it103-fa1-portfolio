...
import { Sun, Moon, ExternalLink, Award, Code, GraduationCap } from "lucide-react";
      ...
      <div className="min-h-screen bg-background text-foreground px-4 py-8 md:px-32 md:py-16 transition-colors duration-500 font-sans">
        ...
        <div className="sticky top-4 z-50 flex justify-end max-w-5xl mx-auto mb-8 pointer-events-none">
          ...
          >
            {darkMode ? (
              <>
                <Sun size={20} className="text-accent-orange animate-spin-slow" />
                <span>Light Mode</span>
              </>
            ) : (
              <>
                <Moon size={20} className="text-zinc-500 group-hover:text-accent-orange" />
                <span>Dark Mode</span>
              </>
            )}
          </button>
        ...
        <main className="max-w-5xl mx-auto flex flex-col gap-10 md:gap-16">
          ...
          <section className="p-6 md:p-12 rounded-2xl md:rounded-3xl border shadow-2xl border-emerald-100 dark:border-emerald-900/30 bg-gradient-to-br from-background via-background to-accent-emerald/10">
            ...
            <ul className="space-y-6 md:space-y-10">
              <li className="flex gap-4 items-start">
                <Award className="text-accent-emerald shrink-0 mt-1" size={28} />
                <div className="flex flex-col">
                  <span className="text-xl md:text-3xl font-bold">Java IT Specialist</span>
                  <span className="text-base md:text-xl text-accent-emerald font-bold">2025</span>
                </div>
              </li>
              <li className="flex gap-4 items-start opacity-90">
                <Award className="text-zinc-400 shrink-0 mt-1" size={28} />
                <div className="flex flex-col">
                  <span className="text-xl md:text-3xl font-semibold">Dean's Lister</span>
              ...
              {[
                ...
                <div key={i} className="group border-l-2 border-transparent hover:border-accent-orange pl-4 transition-all">
                  <div className="flex flex-wrap items-center gap-3">
                    <h3 className={`text-xl md:text-3xl font-bold ${proj.highlight ? "text-accent-orange" : "opacity-90"}`}>
                      {proj.title}
                    </h3>
                    {proj.status && (
          ...
          <section className="p-6 md:p-12 rounded-2xl md:rounded-3xl bg-zinc-950 text-white dark:bg-zinc-900/40 dark:border dark:border-zinc-800 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
              {[
                { label: "FA2", title: "GreenSense", url: "https://dlgaquilon.github.io/it103-fa2-greensense-landing-page/" },
                { label: "FA3", title: "Machine Problems", url: "https://dlgaquilon.github.io/IT103_FA3/" },
                { label: "FA4", title: "Tailwind Blog", url: "https://it103-fa4-4px7m2wo7-dlgaquilons-projects.vercel.app/" },
                { label: "FA5", title: "The Cozy Corner", url: "https://the-cozy-corner-by-belandres.vercel.app/" },
                { label: "FA6", title: "PokeAPI Demo", url: "https://restapi-pokedemo-belandres.vercel.app/" },
              ].map((item, idx) => (
                <a key={idx} href={item.url} target="_blank" className="group flex flex-col gap-2">
                  <span className="text-zinc-500 text-[10px] md:text-xs font-black uppercase tracking-widest">{item.label}</span>
